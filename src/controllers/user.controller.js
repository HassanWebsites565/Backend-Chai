...
const registerUser = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    console.log("Fetching created user details...");
    const createdUser = await User.findById(user._id).select(
      "-password -refreshToken"
    );
    console.log("Created user details fetched:", createdUser);
    ...
    next(new ApiError(500, error.message, true, error.stack));
  }
});
...
const updateUserAvatar = asyncHandler(async(req, res) => {
  ...
  if (!avatar.url) {
    throw new ApiError(400, "Error while uploading on cloudinary of avatar")
...
const updateUserCoverImage = asyncHandler(async(req, res) => {
  ...
  if (!coverImage.url) {
    throw new ApiError(400, "Error while uploading on cloudinary of cover Image file")
